...
<div class="async-select async-select-bootstrap " id="las-ui-{{ $ui }}" data-las-ui="{{ ucfirst($ui) }}" data-las-locale="{{ $locale }}" dir="{{ $this->isRtl ? 'rtl' : 'ltr' }}" >
    <div class="async-select-container position-relative w-100 " x-data="asyncSelect({ multiple: {{ $this->multiple ? 'true' : 'false' }}, tags: {{ $this->tags ? 'true' : 'false' }} })" x-on:click.outside="close()" x-on:keydown.escape.window="close()" x-on:keydown.arrow-down.prevent="highlightNext()" x-on:keydown.arrow-up.prevent="highlightPrevious()" x-on:keydown.enter.prevent="handleEnter()" x-on:keydown.tab="if (!handleTab()) { $event.preventDefault(); }" x-effect="if (highlighted >= optionCount()) { highlighted = Math.max(optionCount() - 1, 0); }" >
        @if ($name)
            @if ($this->multiple)
        ...
        <div class="async-select-input-wrapper position-relative w-100">
            <div class="async-select-input-group input-group ">
                <div class="async-select-trigger form-control d-flex align-items-center flex-wrap" style="min-height: 43px; cursor: text;" x-on:click="if ($refs.search) { $refs.search.focus(); openDropdown(); } else { openDropdown(); }" >
                    @if ($this->multiple)
                        @foreach ($selectedOptions as $chip)
                            ...
                            <span class="async-select-chip badge p-2 text-bg-light mb-1 d-inline-flex align-items-center {{ $this->isRtl ? 'ms-1' : 'me-1' }}" style="gap: 0.25rem;" data-selected="{{ $chipValue }}" wire:key="async-select-chip-{{ md5($chipValue) }}" >
                                ...
                                <button type="button" class="btn-close btn-close-dark {{ $this->isRtl ? 'me-1' : 'ms-1' }}" style="font-size: 0.65rem;" wire:click="clearSelection({{ Js::from($chipValue) }})" x-on:click.stop aria-label="{{ __('async-select::async-select.remove') }}" ></button>
                            </span>
                        @endforeach
                        @if ($this->searchable)
                            <input class="border-0 bg-transparent p-0" style="min-width: 120px; outline: none; flex: 1 1 auto;" x-ref="search" type="text" wire:model.live.debounce.300ms="search" placeholder="{{ count($selectedOptions) ? '' : $placeholder }}" x-on:click="openDropdown()" autocomplete="off" >
                        @endif
                    @else
                        ...
                        @else
                            @if ($this->searchable)
                            <input class="border-0 bg-transparent p-0 w-100" style="outline: none;" x-ref="search" type="text" wire:model.live.debounce.300ms="search" placeholder="{{ $placeholder }}" x-on:click="openDropdown()" autocomplete="off" >
                            @endif
                        @endif
                    @endif
                </div>
                ...
                @if ($this->suffixButton)
                    <button class="async-select-suffix-button btn btn-icon btn-primary d-flex align-items-center justify-content-center {{ !$this->isRtl ? 'rounded-end rounded-start-0' : 'rounded-start rounded-end-0' }}" style="min-height: 43px; white-space: nowrap; flex-shrink: 0;" type="button" x-on:click.stop.prevent="handleSuffixButtonClick($event)" title="{{ __('async-select::async-select.add') }}" aria-label="{{ __('async-select::async-select.add') }}" >
                        @if ($this->suffixButtonIcon)
                            <span class="d-inline-flex align-items-center justify-content-center">{!! $this->suffixButtonIcon !!}</span>
                        @else
                            <svg class="async-select-suffix-icon" style="width: 1.25rem; height: 1.5rem !important; display: inline-block !important; vertical-align: middle; flex-shrink: 0; pointer-events: none;" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg" > <path d="M7.5 3.5V11.5M3.5 7.5H11.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" fill="none"/> </svg>
                        @endif
                    </button>
                @endif
            </div>

            <div x-show="open && !(tags && multiple)" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" x-cloak class="async-select-dropdown form-selec position-absolute w-100 mt-1 rounded border shadow-lg" style="z-index: 1050; top: 100%; left: 0; right: 0;" >
                @if (! $this->multiple && $this->hasSelection && $this->searchable)
                    <div class="mb-1 px-1">
                        <input class="form-control form-control-sm" style="{{ $this->isRtl ? 'text-align: right; direction: rtl;' : '' }}" x-ref="searchDropdown" type="text" wire:model.live.debounce.300ms="search" placeholder="{{ __('async-select::async-select.search') }}" autocomplete="off" >
                    </div>
                @endif
                ...
                <div class="" x-ref="options" role="listbox" x-on:scroll.debounce.150ms=" if ($el.scrollHeight - $el.scrollTop - $el.clientHeight < 50) { if ({{ $hasMore ? 'true' : 'false' }} && !{{ $isLoading ? 'true' : 'false' }}) { $wire.loadMore(); } } " >
                    @if ($this->hasGroups)
                        ...
                        @foreach ($this->groupedOptions as $groupName => $groupOptions)
                            ...
                            @foreach ($groupOptions as $option)
                                ...
                                <div class="async-select-option list-group-item list-group-item-action d-flex align-items-center {{ $isDisabled ? 'disabled' : '' }}" style="gap: 0.5rem;" wire:key="async-select-option-{{ md5($optionValue) }}" :class="{ 'active': highlighted === {{ $globalIndex }} && !{{ $isDisabled ? 'true' : 'false' }}, }" data-option-index="{{ $globalIndex }}" data-value="{{ $optionValue }}" data-selected="{{ $isSelected ? 'true' : 'false' }}" data-disabled="{{ $isDisabled ? 'true' : 'false' }}" role="option" aria-selected="{{ $isSelected ? 'true' : 'false' }}" aria-disabled="{{ $isDisabled ? 'true' : 'false' }}" x-on:mouseenter="if (!{{ $isDisabled ? 'true' : 'false' }}) { highlight({{ $globalIndex }}); }" x-on:click.stop="if (!{{ $isDisabled ? 'true' : 'false' }}) { selectValue({{ Js::from($optionValue) }}); }" >
                                    @if ($slot ?? false)
                                    ...
                                    @else
                                        ...
                                        @if (! $this->multiple && $isSelected)
                                            <svg style="width: 1rem !important; height: 1rem !important; display: inline-block; vertical-align: middle; flex-shrink: 0;" class="text-primary" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M11.5 3.5L6 9L3.5 6.5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/> </svg>
                                        @endif
                                    @endif
                                </div>
                    ...
                    @else
                        @forelse ($this->displayOptions as $index => $option)
                            ...
                            <div class="async-select-option list-group-item list-group-item-action d-flex align-items-center p-2 bg-white {{ $isDisabled ? 'disabled' : '' }}" style="gap: 0.5rem;" wire:key="async-select-option-{{ md5($optionValue) }}" :class="{ 'active': highlighted === {{ $index }} && !{{ $isDisabled ? 'true' : 'false' }}, }" data-option-index="{{ $index }}" data-value="{{ $optionValue }}" data-selected="{{ $isSelected ? 'true' : 'false' }}" data-disabled="{{ $isDisabled ? 'true' : 'false' }}" role="option" aria-selected="{{ $isSelected ? 'true' : 'false' }}" aria-disabled="{{ $isDisabled ? 'true' : 'false' }}" x-on:mouseenter="if (!{{ $isDisabled ? 'true' : 'false' }}) { highlight({{ $index }}); }" x-on:click.stop="if (!{{ $isDisabled ? 'true' : 'false' }}) { selectValue({{ Js::from($optionValue) }}); }" >
                                @if ($this->multiple)
                                    <input
                                        class="form-check-input {{ $this->isRtl ? 'ms-2' : 'me-2' }}"
                                        type="checkbox"
