...
class TaskController extends Controller
{
    static function getTask($active, $client_id, $projet_id, $building_id, $stage_id, $room_id, $search){
        if ($active) {
            if ($client_id) {
                return Task::orderBy('priority_id', 'desc')->where('client_id', $client_id)->finished($search)->paginate(8);
            }
            if ($projet_id) {
                return Task::orderBy('priority_id', 'desc')->where('projet_id', $projet_id)->finished($search)->paginate(8);
            }
            if ($building_id) {
                return Task::orderBy('priority_id', 'desc')->where('building_id', $building_id)->finished($search)->paginate(8);
            }
            if ($stage_id) {
                return Task::orderBy('priority_id', 'desc')->where('stage_id', $stage_id)->finished($search)->paginate(8);
            }
            if ($room_id) {
                return Task::orderBy('priority_id', 'desc')->where('room_id', $room_id)->finished($search)->orderBy('priority_id', 'desc')->paginate(8);
            }
            return Task::finished($search)->paginate(8);
        } else {
            if ($client_id) {
                return Task::orderBy('priority_id', 'desc')->where('client_id', $client_id)->active($search)->paginate(8);
            }
            if ($projet_id) {
                return Task::orderBy('priority_id', 'desc')->where('projet_id', $projet_id)->active($search)->paginate(8);
            }
            if ($building_id) {
                return Task::orderBy('priority_id', 'desc')->where('building_id', $building_id)->active($search)->paginate(8);
            }
            if ($stage_id) {
                return Task::orderBy('priority_id', 'desc')->where('stage_id', $stage_id)->active($search)->paginate(8);
            }
            if ($room_id) {
                return Task::orderBy('priority_id', 'desc')->where('room_id', $room_id)->active($search)->paginate(8);
            }
            return Task::orderBy('priority_id', 'desc')->active($search)->paginate(8);
        }
    }
